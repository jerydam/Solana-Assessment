...
export default function App() {
  ...
  const createSender = async () => {
    ...
    await connection.requestAirdrop(newSenderKeypair.publicKey, LAMPORTS_PER_SOL);

    const latestBlockHash = await connection.getLatestBlockhash();

    // create and send a transaction to confirm the airdrop
    ...
    await sendAndConfirmTransaction(connection, transaction, [newSenderKeypair]);
    ...
    if (solana) {
      try {
        ...
        if (connected) {
          ...
          setReceiverPublicKey(new PublicKey(publicKey.toString()));
        }
      } catch (err) {
        console.log(err);
      }
